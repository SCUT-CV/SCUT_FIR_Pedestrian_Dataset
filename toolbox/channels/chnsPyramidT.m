function pyramid = chnsPyramidT( I, varargin )
...
if( ~isfield(p,'complete') || p.complete~=1 || isempty(I) )
  ...
  p=getPrmDflt(varargin,dfs,1); chns=chnsComputeT([],p.pChns);
...
for i=isR
  ...
  chns=chnsComputeT(I1,pChns); info=chns.info;
...
j=info; 
if(~isempty(j)), j=find(strcmp('color channels',{j.name})); end
