function detector = acfTrainT( varargin )
...
for stage = 0:numel(opts.nWeak)-1
  ...
  if( stage==0 )
    ...
    t=chnsComputeT(t,opts.pPyramid.pChns); detector.info=t.info;
  end
  ...
  if( stage==0 && isempty(opts.pPyramid.lambdas) )
    ...
    ls=chnsScalingT(opts.pPyramid.pChns,IsOrig1,0);
...
p=chnsPyramidT([],opts.pPyramid); p=p.pPyramid;
...
p=chnsPyramidT([],p); p=p.pPyramid; p.complete=1;
...
if( positive ), bbs=gt; bbs=bbs(bbs(:,5)==0,:); else
  if( stage==0 )
    ...
    bbs=acfDetectT(I,detector); [~,ord]=sort(bbs(:,5),'descend');
...
% parfor i=1:n
for i=1:n
  C=chnsComputeT(Is{i},pChns); C=convTri(cat(3,C.data{:}),smooth);
